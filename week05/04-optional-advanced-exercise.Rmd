---
title: "MY472 - Week 5: Optional advanced exercise for scraping unstructured data"
...
```{r}
label_elements_feynman <- html_feynman %>% html_elements(css = _________)
data_elements_feynman <- html_feynman %>% html_elements(css = _________)
```

Next, extract the text from these elements:

```{r}
labels_feynman <- label_elements_feynman %>% _________
data_feynman <- data_elements_feynman %>% _________
```
...
```{r}
data_feynman[_________==_________]
```
...
```{r}
nobel_table_wikipedia <- html_elements(nobels_html, css = _________)[[1]] %>% html_table(fill=TRUE)
...
physicists_names <- nobel_table_wikipedia$Physics %>% str_split(_________) %>% unlist()
...
```{r}
all_link_elements <- html_elements(nobels_html, css = _________)
