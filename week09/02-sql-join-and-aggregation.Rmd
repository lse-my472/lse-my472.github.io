...
```{r}
# Average number of likes and comments, per candidate using a CTE (just for demonstration)
dbGetQuery(db, 
  "WITH 
  ...
  SELECT
    c.screen_name,
    tw.avg_likes_count,
    tw.avg_comments_count
  FROM congress c
  INNER JOIN tweet_metrics tw
  ON c.screen_name = tw.screen_name;
  ")
