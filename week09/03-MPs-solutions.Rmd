...
scotland <- st_crop(gf, ymin = bounds$ymin, ymax = bounds$ymax, xmin = bounds$xmin, xmax = bounds$xmax)
...
```{r}
df <- full_join(gf, rf, by = c("PCON24CD" = "ONS ID"))
```
...
wf <- wf %>% 
  group_by(PCON24CD) %>% 
  mutate(win_margin = round(percent-lead(percent), 4)) %>%
  ungroup() %>%
  select(PCON24CD,PCON24NM,london,percent,win_margin,`Party abbreviation`,geometry)
...
```{r}
z <- ggplot(wf) +
  geom_sf(aes(fill = `Party abbreviation`), color = "black", linewidth = 0.1) + 
...
```{r}
z <- ggplot(wf[wf$london==1, ]) +
  geom_sf(aes(fill = `Party abbreviation`, alpha = win_margin), color = "black", linewidth = 0.1) + 
  scale_fill_manual(values = unlist(cols)) +
...
raw <- read_html(mp.url) # just do the first page
...
url <- raw %>% html_elements(xpath="//div[@class='image']") %>% as.character() %>% str_extract("url[(]([^)]+)[)]", group=1)
id <- str_extract(url,"Members/([0-9]+)", group = 1)
image <- paste0(mdir,"/",id,".jpg")
name <- raw %>% html_elements(xpath="//div[@class='primary-info']") %>% html_text() %>% str_squish()
...
for(i in 1:nrow(df)){
  ...
  download.file(df$url[i], df$image[i], mode="wb")
...
z <- ggplot(tp, aes(x = x, y = y, fill = fill)) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 16, face = "bold"),
        plot.subtitle = element_text(hjust = 0.5, size = 12, face = "italic"), 
        strip.text.x = element_text(size = 6)) + 
  facet_wrap(vars(cols),nrow = 2, ncol = 3)
