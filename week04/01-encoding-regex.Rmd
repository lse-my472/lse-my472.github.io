...
write_file(raw, paste0(root, "hubert1.txt")) # note: default encoding is UTF-8

# Write a plain text file with my last name, but omit the accent
raw <- "Hubert" # how many bytes should you expect?
write_file(raw, paste0(root, "hubert2.txt")) # note: default encoding is UTF-8
```
...
```{r}
## This works
read_file(paste0(root, "utf-examples-8.txt"))  # This works (sort of...)
## These do not work
read_file(paste0(root, "utf-examples-16.txt"))  # This drops most of the characters
read_file(paste0(root, "utf-examples-32.txt"))  # This drops most of the characters
```
...
```{r, error = TRUE}
read_file(paste0(root, "shanghai.txt")) 
```

This does not work at all:

```{r, error = TRUE}
read_file(paste0(root, "shanghai.txt"), locale = locale(encoding="utf-8"))
```

This does not print the same characters:

```{r, error = TRUE}
read_file(paste0(root, "shanghai.txt"), locale = locale(encoding="utf-16")) 
```

Maybe R can help us?? (Nope!)

```{r, error = TRUE}
guess_encoding(paste0(root, "shanghai.txt"))
```
...
```{r}
read_file(paste0(root, "shanghai.txt"), locale = locale(encoding="gb18030"))
```
...
```{r}
shanghai <- read_file(paste0(root, "shanghai.txt"), locale = locale(encoding="gb18030"))
print(shanghai)
write_file(shanghai, paste0(root, "shanghai_utf-8.txt"))
```

Now that we've saved with UTF-8 encoding, let's see how much easier it is to read the file:

```{r}
read_file(paste0(root, "shanghai_utf-8.txt"))
```
