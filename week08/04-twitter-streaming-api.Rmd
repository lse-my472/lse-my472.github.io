...
To access the streaming API with `rtweet`, we again need to authenticate. 

__Option 1:__

One option for the streaming API is to authenticate via keys/tokens from the Twitter Developer Portal. This works with accounts with Academic Access, but other accounts currently do not seem to have the required access rights. If you go to your Twitter Developer Portal, you can create and copy:
...
If the follwing cell runs without a 403 error, we are good to go:
...
Another option is to use `rtweet's` function `auth_setup_default()` to authenticate as a user instead. This will open a browser window and require to enter Twitter account name and password. Note that this will share information of your account with the `rtweet` package and its associated Twitter app. For more information on what is shared, see the description in the browser window. It is therefore only optional in this course to use the Twitter API and up to the user to choose this authentication or not. No assignment in this course requires students to download data from the Twitter API.

For more in detail documentation about `rtweet's` current authentication, also see this link: https://docs.ropensci.org/rtweet/articles/auth.html
