...
```{r}
apikey <- "u8lXp0divbjWlHtaEbWEvFQqsMK7iOaa"
```

Make sure not to display the Rmd or HTML file publicly, e.g. in a public repo, when it contains an active key. In the following notebook, we will discuss an alternative approach that avoids keys being visible in code files.
...
This seems to work as well. Next, we run this function for 50 years and plot the outcome (this takes some time to run):
