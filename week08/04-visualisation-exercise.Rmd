...
yrange <- range(unemp$date, na.rm=TRUE)

# Fix axes and add colored rectangles
p <- p + scale_x_date(limits= xrange, expand = c(0,0))
...
p <- p + geom_text(data = pres, aes(x=start,y=yrange[2], label=name, angle=90))
...
#- means look at the dataset (equal to df_ploy$likes_count)

fig <- plot_ly(df_plot, x=df_plot$likes_count, y=df_plot$shares_count, z= df_plot$message_length, color = ~party)
fig <- fig %>% add_markers()
