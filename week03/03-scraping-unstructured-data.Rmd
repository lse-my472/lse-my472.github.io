...
bribe_html <- ________(url)

# Identify the CSS selector and add it to the css argument
bribe_nodes <- html_nodes(bribe_html, css = "___________")
...
bribe_amount <- ________(bribe_nodes) %>%
...
```{r}
transaction <- html_nodes(bribe_html, "_____________") %>% 
  html_text
