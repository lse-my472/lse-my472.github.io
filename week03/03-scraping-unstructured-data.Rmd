...
```{r}
transaction <- html_nodes(bribe_html, css=".transaction") %>%
  html_text
...
# also add 'a' which gives RStudio more infO and the it returns only that 'a' type within the selected class, not the extra infos that are not useful for our purposes
...
```{r}
# base_url_my <- "http://ipaidabribe.com/reports/all?page="
...
```{r}
# frequency table
frequencies <- table(data_all$transaction)
frequencies
# hist(frequencies)
