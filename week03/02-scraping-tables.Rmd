...
# tab is a list containing all tables from the website
...
social_security_data <- tab[[1]] # first (and only) table in our list of tables assigned to social_security_data
...
social_security_data_long_my <- pivot_longer(data = social_security_data, cols = c('male', 'female'),
                                             names_to = 'gender', values_to = 'individuals')
...
```{r}
tables_raw_my <- html_nodes(html, css= '#mw-content-text > div.mw-parser-output > table:nth-child(18)')
...
# colnames(data_pop) <- c('city', 'pop', 'ran') # my alternative version, renaming in order
